...
static int run_codex_agent_request(const std::string &request_path, const std::string &response_path)
{
    namespace bp = boost::process;

    boost::filesystem::path script_path = boost::filesystem::path(resources_dir()) / "agent" / "bambu_agent.py";
    if (!boost::filesystem::exists(script_path))
        script_path = boost::filesystem::current_path() / "resources" / "agent" / "bambu_agent.py";

    if (!boost::filesystem::exists(script_path)) {
        boost::nowide::cerr << "Agent script not found: " << script_path.string() << std::endl;
        return CLI_FILE_NOTFOUND;
    }

    std::vector<std::string> python_candidates;
    if (const char *env_python = boost::nowide::getenv("BAMBU_AGENT_PYTHON"); env_python && *env_python)
        python_candidates.emplace_back(env_python);
    python_candidates.emplace_back("python3");
    python_candidates.emplace_back("/opt/homebrew/bin/python3");
    python_candidates.emplace_back("/usr/bin/python3");

    boost::filesystem::path python_path;
    for (const std::string &candidate : python_candidates) {
        boost::filesystem::path p(candidate);
        if (p.is_absolute()) {
    ...
    if (python_path.empty()) {
        boost::nowide::cerr << "python3 not found for Bambu agent automation." << std::endl;
        return CLI_ENVIRONMENT_ERROR;
    }

    try {
        bp::child child(python_path.string(), script_path.string(), "--request", request_path, "--response", response_path);
        child.wait();
        return child.exit_code();
    } catch (const std::exception &e) {
        boost::nowide::cerr << "Failed to run Bambu agent automation: " << e.what() << std::endl;
        return CLI_ENVIRONMENT_ERROR;
    }
}
...
int CLI::run(int argc, char **argv)
{
    ...
    const ConfigOptionString *agent_run_option = m_config.option<ConfigOptionString>("agent_run");
    if (agent_run_option && !agent_run_option->value.empty()) {
        const ConfigOptionString *agent_out_option = m_config.option<ConfigOptionString>("agent_out");
        std::string agent_out = agent_out_option && !agent_out_option->value.empty() ?
            agent_out_option->value :
            agent_run_option->value + ".out.json";
        return run_codex_agent_request(agent_run_option->value, agent_out);
    }
